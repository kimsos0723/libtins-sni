...
std::string Extentions_t::server_names() {    
    ...
    for (size_t i = 0; i < bytes.size(); i++) {
        if (catUint8(bytes[i], bytes[i + 1]) == 0x0000) {
            sni.snType = catUint8(bytes[i], bytes[i + 1]);
        	sni.len = catUint8(bytes[i+2], bytes[i + 3]);
            sni.snlistLen = catUint8(bytes[i + 4], bytes[i + 5]);
            sni.snType = bytes[i + 6];
            sni.snlen = catUint8(bytes[i + 7], bytes[i + 8]);
